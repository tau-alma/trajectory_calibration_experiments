...
path = './simulation_tests/outliers/'
subfolders = glob(path + 'outliers_*')
...
for folder in subfolders:
    outliers = float(folder.split('_')[-1])

    for f in glob(folder + '/run_*'):
...
df.to_csv('calib/simulation_tests_outliers.csv')
