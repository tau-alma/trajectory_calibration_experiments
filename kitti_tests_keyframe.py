...
path = './kitti_tests/2011_09_30_drive_0027/'
...
GT = calibration.utils.io.get_trajectories(
    [path + 'camera_gray_left_in_velo_link.txt'])[0].as_array().squeeze()
trajectories = calibration.utils.io.get_trajectories(
    [path + 'LIDAR_HDL64E/traj_HDL_aft_opt.csv',
     path + 'cam01/KeyFrameTrajectory_TUM_Format.txt'], estimate=False)

# Removing the first point from camera trajectory is necessary for
# the interpolation in this case
...
df.to_csv('calib/kitti_tests_keyframe.csv')
